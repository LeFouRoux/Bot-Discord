...
module.exports = {
    ...
    subcommands: [
      {
        ...
        description: "❓ ❱ Activer ou désactiver le message d'au revoir",
      },
      {
      ...
      {
        trigger: "preview",
        description: "🤖 ❱ Prévisualiser le message d'au revoir configuré",
      },
      {
        trigger: "desc <text>",
      ...
      },
      {
        ...
        description: "❓ ❱ Activer/désactiver une vignette a l'embed",
      },
      {
  ...
  slashCommand: {
    ...
    options: [
      ...
      {
        name: "channel",
        description: "🤖 ❱ Définir le salon au serra envoyer l'embed",
        type: ApplicationCommandOptionType.Subcommand,
        options: [
          {
            name: "channel",
            description: "❓ ❱ Nom du salon au serra envoyer l'embed",
  ...
  async messageRun(message, args, data) {
    ...
    // status
      ...
      if (!status || !["ON", "OFF"].includes(status))
        return message.safeReply("Statut invalide. La valeur doit être `Activité/Désactivité`");
...
async function sendPreview(settings, member) {
  if (!settings.farewell?.enabled) return "Message d'au revoir non activé sur ce serveur";

  const targetChannel = member.guild.channels.cache.get(settings.farewell.channel);
  if (!targetChannel) return "Aucun canal n'est configuré pour envoyer un message d'au revoir";

  const response = await buildGreeting(member, "FAREWELL", settings.farewell);
  await targetChannel.safeSend(response);

  return `Envoyé un aperçu d'au revoir à ${targetChannel.toString()}`;
}
...
async function setStatus(settings, status) {
  ...
  await settings.save();
  return `Configuration enregistrée ! Un message d'au revoir ${status ? "Activité" : "Désactivité"}`;
}
...
async function setChannel(settings, channel) {
  ...
  await settings.save();
  return `Configuration enregistrée ! Un message d'au revoir sera envoyé à ${channel ? channel.toString() : "Pas trouvé"}`;
}

async function setDescription(settings, desc) {
  settings.farewell.embed.description = desc;
  await settings.save();
  return "Configuration enregistrée ! Message d'au revoir mis à jour";
}
