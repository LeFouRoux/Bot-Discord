...
module.exports = {
  name: "listeserveurs",
  description: "🤴 ❱ Répertorie tous les serveurs.",
  category: "OWNER",
  ...
  async messageRun(message, args) {
    ...
    if (totalPages === 0) return message.safeReply("Aucun serveur trouvé");
    let currentPage = 1;
    ...
    const buildEmbed = () => {
      ...
      const embed = new EmbedBuilder()
        ...
        .setAuthor({ name: "Liste des serveurs" })
        .setFooter({ text: `${match ? "Matched" : "Total"} Serveurs: ${total} • Page ${currentPage} sur ${totalPages}` });
