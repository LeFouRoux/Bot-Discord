...
const logModeration = async (issuer, target, reason, type, data = {}) => {
  ...
  switch (type.toUpperCase()) {
    case "PURGE":
      embed.setAuthor({ name: `Modération - ${type}` });
      fields.push(
        { name: "Type de purge", value: data.purgeType, inline: true },
  ...
  if (type.toUpperCase() !== "PURGE") {
    embed.setAuthor({ name: `Modération - ${type}` }).setThumbnail(target.displayAvatarURL());
    ...
    fields.push({ name: "Raison", value: reason || "Aucune raison fournie", inline: false });
