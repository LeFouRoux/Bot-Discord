...
module.exports = {
  ...
  handlePrefixCommand: async function (message, cmd, settings) {
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(message.author.id)) {
      return message.safeReply("Cette commande n'est accessible qu'aux propriétaires de bot");
    }
    ...
    if (cmd.userPermissions && cmd.userPermissions?.length > 0) {
      if (!message.channel.permissionsFor(message.member).has(cmd.userPermissions)) {
        return message.safeReply(`Vous avez besoin ${parsePermissions(cmd.userPermissions)} pour cette commande`);
      }
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      if (!message.channel.permissionsFor(message.guild.members.me).has(cmd.botPermissions)) {
        return message.safeReply(`J'ai besoin ${parsePermissions(cmd.botPermissions)} pour cette commande`);
      }
    }
    ...
    if (cmd.cooldown > 0) {
      ...
      if (remaining > 0) {
        return message.safeReply(`Vous êtes en recharge. Vous pouvez à nouveau utiliser la commande dans \`${timeformat(remaining)}\``);
      }
    }
    ...
    try {
      ...
      message.safeReply("Une erreur s'est produite lors de l'exécution de cette commande");
    } finally {
  ...
  handleSlashCommand: async function (interaction) {
    ...
    if (!cmd) return interaction.reply({ content: "Une erreur est survenue", ephemeral: true }).catch(() => {});
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(interaction.user.id)) {
      return interaction.reply({
        content: `Cette commande n'est accessible qu'aux propriétaires de bot`,
        ephemeral: true,
      });
    }
    ...
    if (interaction.member && cmd.userPermissions?.length > 0) {
      if (!interaction.member.permissions.has(cmd.userPermissions)) {
        return interaction.reply({
          content: `Vous avez besoin ${parsePermissions(cmd.userPermissions)} pour cette commande`,
          ephemeral: true,
        });
      }
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      if (!interaction.guild.members.me.permissions.has(cmd.botPermissions)) {
        return interaction.reply({
          content: `J'ai besoin ${parsePermissions(cmd.botPermissions)} pour cette commande`,
          ephemeral: true,
        });
      }
    }
    ...
    if (cmd.cooldown > 0) {
      ...
      if (remaining > 0) {
        return interaction.reply({
          content: `Vous êtes en recharge. Vous pouvez à nouveau utiliser la commande dans \`${timeformat(remaining)}\``,
          ephemeral: true,
        });
      }
    }
    ...
    try {
      ...
      await interaction.followUp("Oops! Une erreur s'est produite lors de l'exécution de la commande");
