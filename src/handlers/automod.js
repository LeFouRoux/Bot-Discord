...
async function performAutomod(message, settings) {
  ...
  if (automod.anti_massmention > 0) {
    ...
    if (mentions.users.size + mentions.roles.size > automod.anti_massmention) {
      fields.push({
        name: "Mentions d'utilisateur/rôle",
  ...
  if (automod.max_lines > 0) {
    ...
    if (count > automod.max_lines) {
      fields.push({ name: "Nouvelles lignes", value: `${count}/${automod.max_lines}`, inline: true });
      shouldDelete = true;
  ...
  if (automod.anti_attachments) {
    if (message.attachments.size > 0) {
      fields.push({ name: "Pièces jointes trouvées", value: "✓", inline: true });
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
  ...
  if (automod.anti_links) {
    if (containsLink(content)) {
      fields.push({ name: "Liens trouvés", value: "✓", inline: true });
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
    ...
    if (containsLink(content)) {
      ...
      if (antispamCache.has(key)) {
        ...
        if (
          ...
          fields.push({ name: "Détection antispam", value: "✓", inline: true });
          shouldDelete = true;
          strikesTotal += 1;
        }
      } else {
  ...
  if (!automod.anti_links && automod.anti_invites) {
    if (containsDiscordInvite(content)) {
      fields.push({ name: "Discord invite", value: "✓", inline: true });
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
  ...
  if (shouldDelete && message.deletable) {
    message
      .delete()
      .then(() => channel.safeSend("> Modération automatique ! Message supprimé", 5))
      .catch(() => {});
  }
  ...
  if (strikesTotal > 0) {
    ...
    const strikeEmbed = new EmbedBuilder()
      ...
      .addFields(fields)
      .setDescription(
        `Vous avez reçu ${strikesTotal} avertisement(s)!\n\n` +
          `**Serveur:** ${guild.name}\n` +
          `**Total d'avertisement:** ${memberDb.strikes} out of ${automod.strikes}`
      );
    ...
    if (memberDb.strikes >= automod.strikes) {
      ...
      await addModAction(guild.members.me, member, "Automod : nombre maximal d'avertissements reçus", automod.action).catch(() => {});
    }
